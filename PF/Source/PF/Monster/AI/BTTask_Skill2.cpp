...
EBTNodeResult::Type UBTTask_Skill2::AbortTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
...
void UBTTask_Skill2::TickTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, float DeltaSeconds)
{
	...
	if (Monster->GetMonsterPhase() != EMonsterPhase::Skill2)
	{
		FinishLatentTask(OwnerComp, EBTNodeResult::Failed);
		return;
	}
